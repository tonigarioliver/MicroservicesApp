...
export class MeasurementTypeApiService {
  private baseUrl = 'http://localhost:8001/api/v1/device-measurement-type'; // Reemplaza con la URL de tu API
  ...
  getMeasurementTypes(): Observable<MeasurementType[]> {
    ...
    return this.http.get<{ measurementTypes: MeasurementType[] }>(url)
      .pipe(
  ...
  updateMeasurementType(measurementType: MeasurementType): Observable<MeasurementType> {
    const url = `${this.baseUrl}/measurement-types/${measurementType.measurementTypeId}`;
  ...
  deleteMeasurementType(measurementTypeId: number): Observable<void> {
    const url = `${this.baseUrl}/measurement-types/${measurementTypeId}`;
